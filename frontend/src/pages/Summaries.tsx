      ...
      <div className="grid gap-6">
          ...
          <div key={summary.id} className="bg-white shadow rounded-lg">
            <div className="px-4 py-5 sm:p-6">
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 gap-2">
                <h3 className="text-lg font-medium text-gray-900 break-words">{summary.title}</h3>
              ...
              <div className="prose max-w-none prose-sm prose-headings:break-words prose-p:break-words">
                <ReactMarkdown 
                  ...
                  components={{
                    ...
                    th: ({ children }: any) => (
                      <th className="px-4 py-3 bg-gray-50 text-left text-xs font-medium text-gray-700 uppercase tracking-wider break-words">
                        {children}
                      </th>
                    ),
                    td: ({ children }: any) => (
                    ...
                    // 自定义标题样式 - 添加换行支持
                    h1: ({ children }: any) => (
                      <h1 className="text-2xl font-bold text-gray-900 mt-6 mb-4 break-words">
                        {children}
                      </h1>
                    ),
                    h2: ({ children }: any) => (
                      <h2 className="text-xl font-semibold text-gray-900 mt-5 mb-3 break-words">
                        {children}
                      </h2>
                    ),
                    h3: ({ children }: any) => (
                      <h3 className="text-lg font-medium text-gray-900 mt-4 mb-2 break-words">
                        {children}
                      </h3>
                    ),
                    // 自定义段落样式 - 添加换行支持
                    p: ({ children }: any) => (
                      <p className="text-gray-700 leading-relaxed break-words mb-4">
                        {children}
                      </p>
                    ),
                    // 自定义引用样式
                    blockquote: ({ children }: any) => (
                      <blockquote className="border-l-4 border-gray-300 pl-4 italic text-gray-600 my-4 break-words">
                        {children}
                      </blockquote>
                    ),
                    // 自定义链接样式
              ...
              <div className="mt-4 flex flex-col sm:flex-row sm:items-center sm:justify-between text-sm text-gray-500 gap-2">
                <div className="flex flex-wrap items-center gap-4">
                  <span className="whitespace-nowrap">{language === 'chinese' ? '成员数' : 'Members'}: {summary.member_count}</span>
                  <span className="whitespace-nowrap">{language === 'chinese' ? '活动数' : 'Activities'}: {summary.activity_count}</span>
                  {summary.start_date && (
                    <span className="whitespace-nowrap">{language === 'chinese' ? '开始' : 'Start'}: {new Date(summary.start_date).toLocaleDateString()}</span>
                  )}
                  {summary.end_date && (
                    <span className="whitespace-nowrap">{language === 'chinese' ? '结束' : 'End'}: {new Date(summary.end_date).toLocaleDateString()}</span>
                  )}
                </div>
                <div className="flex flex-wrap items-center gap-2">
                  <span className="whitespace-nowrap">{language === 'chinese' ? '创建时间' : 'Created'}: {new Date(summary.created_at).toLocaleString()}</span>
                  {summary.is_sent && (
                    <span className="text-green-600 whitespace-nowrap">{language === 'chinese' ? '已发送' : 'Sent'}</span>
                  )}
                </div>
              </div>
            </div>
        ...
        {summaries.length === 0 && (
          <div className="text-center py-12 bg-white shadow rounded-lg">
            <p className="text-gray-500 break-words px-4">
              {language === 'chinese' ? '暂无总结报告' : 'No summary reports available'}
            </p>
            <button
              onClick={generateDailySummary}
              disabled={generating}
              className="mt-4 bg-primary-600 hover:bg-primary-700 disabled:bg-gray-400 text-white px-4 py-2 rounded-lg whitespace-nowrap"
            >
              {generating ? 
